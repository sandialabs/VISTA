...
# Separate router for ML pipeline mutation endpoints that require HMAC auth.
# These should ONLY be mounted on the /api-ml prefix, not on /api or /api-key.
ml_pipeline_router = APIRouter(tags=["ML Pipeline"])
...
@ml_pipeline_router.patch("/analyses/{analysis_id}/status", response_model=schemas.MLAnalysis)
...
@ml_pipeline_router.post("/analyses/{analysis_id}/annotations:bulk", response_model=schemas.MLAnnotationList)
...
@ml_pipeline_router.post("/analyses/{analysis_id}/artifacts/presign", response_model=PresignResponse)
...
@ml_pipeline_router.post("/analyses/{analysis_id}/finalize", response_model=schemas.MLAnalysis)
